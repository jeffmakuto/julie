...
class EmailPollingAppS3:
    """
    FastAPI app with background email polling and S3-based RAG ingestion.
    """
    ...
    async def read_root(self) -> dict[str, str]:
        return {"message": "Email polling service running with S3 ingestion."}

    @asynccontextmanager
    async def lifespan(self, app: FastAPI):
        """
        Startup: RAG runner + S3 KB preload + async email polling.
        """
        try:
            ...
            # Inject RAG runner into pipeline
            self.polling_service.pipeline.rag = rag_runner

            # Run the async poller in the background
            ...
            logger.info("Shutting down polling service and scheduler...")
            if self.scheduler:
